...
async function getTransparentLogo(browser) {
  if (_logoTransparent) return _logoTransparent;
  const b64 = getLogoRawB64();
  const page = await browser.newPage();
  try {
    _logoTransparent = await page.evaluate((src) => new Promise((resolve, reject) => {
      const img = new Image();
      img.onload = () => {
...
function logo(logoDataUrl, w = 280, extra = '') {
  return `<img src="${logoDataUrl}" style="width:${w}px;display:block;${extra}" />`;
}
...
function footer(onDark = true) {
  return `<div style="position:absolute;bottom:0;left:0;right:0;height:88px;background:${onDark ? NAVY : 'rgba(13,14,42,0.97)'};display:flex;align-items:center;justify-content:space-between;padding:0 32px;z-index:30;gap:10px">
    ...
    <div style="width:2px;height:44px;background:${ORANGE};opacity:0.35;flex-shrink:0"></div>
    <div style="display:flex;align-items:center;gap:9px;flex-shrink:0">
      <svg viewBox="0 0 24 24" width="26" height="26" fill="${ORANGE}" style="flex-shrink:0"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/></svg>
      <span style="color:${WHITE};font-family:'Barlow Condensed',sans-serif;font-weight:700;font-size:22px;white-space:nowrap">${SITE}</span>
    </div>
    <div style="width:2px;height:44px;background:${ORANGE};opacity:0.35;flex-shrink:0"></div>
    <div style="display:flex;align-items:center;gap:10px;flex-shrink:0">
      ${WA_ICON}
      <div style="display:flex;flex-direction:column;line-height:1.25">
        <span style="color:${WHITE};font-family:'Barlow Condensed',sans-serif;font-size:22px;font-weight:700;white-space:nowrap">${PHONE}</span>
        <span style="color:rgba(255,255,255,0.6);font-family:'Barlow',sans-serif;font-size:16px;white-space:nowrap">${EMAIL}</span>
      </div>
    </div>
  </div>`;
}
...
const TEMPLATES = [
  ...
  ({ img, h, sub, logoDataUrl }) => wrap(`
    <div style="width:1080px;height:1080px;background:url('${img}') center/cover no-repeat;position:relative">
      ...
      <div style="position:absolute;top:32px;left:36px">${logo(logoDataUrl, 280)}</div>
  ...
  ({ img, h, sub, logoDataUrl }) => wrap(`
    <div style="width:1080px;height:1080px;background:url('${img}') center/cover no-repeat;position:relative">
      ...
      <div style="position:absolute;top:16px;right:16px">${logo(logoDataUrl, 260)}</div>
  ...
  ({ img, h, sub, logoDataUrl }) => wrap(`
    <div style="width:1080px;height:1080px;background:url('${img}') center/cover no-repeat;position:relative">
      <div style="position:absolute;top:32px;right:36px">${logo(logoDataUrl, 280)}</div>
  ...
  ({ img, h, sub, logoDataUrl }) => wrap(`
    <div style="width:1080px;height:1080px;display:flex;flex-direction:column;position:relative">
      <div style="height:58%;background:url('${img}') center/cover no-repeat;position:relative">
        <div style="position:absolute;top:28px;left:32px">${logo(logoDataUrl, 280)}</div>
  ...
  ({ img, h, sub, logoDataUrl }) => wrap(`
    <div style="width:1080px;height:1080px;display:flex;position:relative">
      ...
      <div style="flex:1;background:url('${img}') center/cover no-repeat;position:relative">
        ...
        <div style="position:absolute;top:28px;right:32px">${logo(logoDataUrl, 280)}</div>
  ...
  ({ img, h, sub, logoDataUrl }) => wrap(`
    <div style="width:1080px;height:1080px;background:url('${img}') center/cover no-repeat;position:relative">
      ...
      <div style="position:absolute;top:36px;left:50%;transform:translateX(-50%)">${logo(logoDataUrl, 280)}</div>
  ...
  ({ img, h, sub, logoDataUrl }) => wrap(`
    <div style="width:1080px;height:1080px;background:url('${img}') center/cover no-repeat;position:relative">
      ...
      <div style="position:absolute;top:40px;left:40px;z-index:2">${logo(logoDataUrl, 280)}</div>
...
export async function renderBrandedImage({ imageUrl, headline, sub, postCount }) {
  ...
  try {
    const [imgDataUrl, logoDataUrl] = await Promise.all([
      fetchImageAsDataUrl(imageUrl),
      getTransparentLogo(browser),
    ]);

    const html = templateFn({ img: imgDataUrl, h: headline, sub: sub || '', logoDataUrl });
